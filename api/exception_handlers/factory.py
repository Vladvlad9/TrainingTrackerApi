...
class ExceptionHandlerFactory:
    """
    A factory for creating exceptions handler decorators. It maps exceptions to custom exceptions
    and applies the mapping during function execution.

    :param exc_mapping: A dictionary where keys are source exceptions types and values are custom exceptions types or instances.
    :type exc_mapping: dict[ExceptionClass, ExceptionClass | Exception]
    :param default_exc: The default exceptions type to raise if no mapping is found.
    ...
    def __call__(self, exc_override: dict[ExceptionClass, ExceptionClass | Exception] | None = None) -> Callable:
        """
        Creates a decorator that applies the exceptions-to-custom-exceptions mapping.

        :param exc_override: An optional dictionary to override the default exceptions mappings.
